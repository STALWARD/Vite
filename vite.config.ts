    ...
    output: {
      manualChunks(id) {
        if (id.includes('node_modules')) {
          // 1. Framework: The absolute bare minimum (React + Router)
          if (id.includes('react/') || id.includes('react-dom/') || id.includes('react-router/')) {
            return 'vendor-framework';
          }
          
          // 2. Blog Logic: Heavy Node polyfills (Isolate from main bundle)
          ...
          // This keeps 'vendor-framework' pure and small
          return 'vendor-libs';
        }
      },
    },
